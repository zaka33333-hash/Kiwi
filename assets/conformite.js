        ...
        const rows = tasks.map((tk) => {
          ...
          return `
            <div class="cf-hyg-task${done ? ' is-done' : ''}" data-action="cf-hyg-toggle" data-task="${tk.id}">
              ...
              <span class="cf-hyg-label">${esc(tk.label)}</span>
    ...
    const rows = list.map((it) => {
      ...
      return `
        <div class="cf-eq-row" data-action="cf-eq-detail" data-eq="${it.id}">
          <span class="cf-eq-name">${esc(it.name)}</span>
          <span class="cf-eq-type">${esc(it.type)}</span>
          <span class="cf-eq-last">${esc(it.last)}</span>
          <span class="cf-eq-next">${esc(it.next)} <small>· ${esc(it.cadence)}</small></span>
    ...
    const hist = (item.history || []).length
      ? item.history.map((h) => `
        <div class="cf-eq-hist-row">
          ...
          <span class="cf-eq-hist-what">${esc(h.what)}</span>
